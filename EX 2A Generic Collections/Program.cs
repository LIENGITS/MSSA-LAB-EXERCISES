...
namespace EX_2A_GenericCollections
{
    class Program
    {
        static void Main(string[] args)
        {
            Console.WriteLine("Please modify a previous key-value dictionary to have a generic type parameter.");
            Console.ReadLine();
        }
        ...
        public class Dictionary<T>
        {
            ...
            public void ModifyValue(string indexKey, T value)
            {
                for (int i = 0; i < _dictionaryKV.Length; i++)
                {
                    if (_dictionaryKV[i]._key == indexKey)
